...
O sistema corrige código C de provas como um **funil interpretável**: análise estática (tree-sitter) + execução isolada (Docker/GCC) + heurísticas pedagógicas classificam o erro, e as submissões são **agrupadas por assinatura de falha** para o professor intervir por grupo. Base do aluno já existe (submissão pública + feedback imediato), mas falta conta de aluno, histórico e engajamento.
...
backend/app/
  ...
  api/routes/          — exam.py, submission.py, turma.py, jobs.py
  auth/                — JWT de professor, posse de recursos
  engine/
  ...
  llm/feedback_generator.py — insights por grupo (Gemini, 1 chamada/questão, cacheada)
  services/              — orquestração (exam, submission, bulk, turma, jobs)
  models/                — orm.py (9 entidades) + schemas.py (Pydantic)
  migrations/            — Alembic (revisões 0001..0009)
frontend/                — painel do professor + submissão do aluno (React 19 + Vite + Tailwind v4)
```
...
**Princípio central:** sem "modos de prova". Cada questão declara o que exige (`required_structures`, `forbidden_structures`, `required_functions`) e os verificadores só disparam o relevante (sistema adaptativo).
